...
template <typename IterTyLhs, typename IterTyRhs,
...
static constexpr auto set_equality(IterTyLhs lhs_begin, IterTyLhs lhs_end,
                                   IterTyRhs rhs_begin, IterTyRhs rhs_end)
    -> bool {
  using ValueTy =
...
template <typename IterTyLhs, typename IterTyRhs,
          typename = std::enable_if<std::is_same_v<
              typename IterTyLhs::value_type, typename IterTyRhs::value_type>>>
static constexpr bool set_equality(IterTyLhs lhs, IterTyRhs rhs) {
  return set_equality(std::cbegin(lhs), std::cend(lhs), std::cbegin(rhs),
                      std::cend(rhs));
}
