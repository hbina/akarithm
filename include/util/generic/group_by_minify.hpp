...
template <typename CounterTy = std::size_t, typename IterTy,
...
static constexpr auto group_by_minify(IterTy iter_begin, IterTy iter_end,
                                      const BinaryPredicate &pred) -> ReturnTy {
  if (iter_begin == iter_end)
    return {};
  return std::accumulate(std::next(iter_begin), iter_end,
