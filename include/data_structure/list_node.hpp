...
template <typename ValueTy = int> struct ListNode {
  ValueTy val;
  ListNode *next;

  constexpr ListNode(const ValueTy &x) : val(x), next(nullptr) {}
  constexpr ListNode(const ValueTy &x, ListNode *next) : val(x), next(next) {}
