...
TEST_CASE("zip accumulate 1") {
  ...
  const int result = akarithm::zip_fold(
      std::cbegin(input_1), std::cend(input_1), std::cbegin(input_2),
      std::cend(input_2), 0, std::multiplies{}, std::plus{});
...
TEST_CASE("zip accumulate 2") {
  ...
  const bool result = akarithm::zip_fold(
      std::cbegin(input_1), std::cend(input_1), std::cbegin(input_2),
      std::cend(input_2), true, std::equal_to{}, std::logical_and{});
  CHECK(expected == result);
}
