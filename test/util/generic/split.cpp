...
TEST_CASE("akarithm::split test -- vector<int> --> vector<vector<int>> "
  ...
  const auto result =
      akarithm::split<int>(std::cbegin(input), std::cend(input), delimiter);
...
TEST_CASE("akarithm::split test -- vector<int> --> vector<vector<int>> "
  ...
  const auto result = akarithm::split_if<int>(
      std::cbegin(input), std::cend(input),
      [](const int &elem) -> bool { return elem % 2 == 0; });
  CHECK(expected == result);
}
...
TEST_CASE("util::string::split test -- vector<int> --> vector<vector<int>> "
  ...
  const std::vector<std::string> expected = {"hello", "world", "good", "bye",
                                             "world"};
  const std::vector<std::string> result =
      akarithm::split_if<std::string::value_type, std::string>(
          std::cbegin(input), std::cend(input),
          [](const auto &value) -> bool { return std::isdigit(value); });
  CHECK(expected == result);
}
