...
#include "akarithm/problem_14.hpp"

TEST_CASE("problem_14")
{
    const std::vector<std::string> input = {"flower", "flow", "flight"};
    const std::string expected = "fl";
    const std::string result = akarithm::longestCommonPrefix(input);
    const std::string result_Iterator = akarithm::longestCommonPrefix_Iterator(
        input.cbegin(),
        input.cend());
...
TEST_CASE("problem_14")
{
    ...
    const std::vector<int> result_Iterator = akarithm::longestCommonPrefix_Iterator(
        input.cbegin(),
        input.cend());
    CHECK(expected == result_Iterator);
}
